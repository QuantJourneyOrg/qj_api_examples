...
def plot_options_vol_surface_3d(out: Path) -> None:
    ...
    fig = plt.figure(figsize=(14, 8), facecolor=NAVY)
    title(fig, "Options Surface: Implied Volatility by Strike and Expiry", "Options chain, expirations, underlying price and CBOE volatility context rendered as a 3D volatility surface")
    ...
    cbar = fig.colorbar(surf, ax=ax, fraction=0.035, pad=0.04)
...
def plot_backtest_parameter_surface_3d(out: Path) -> None:
    ...
    fig = plt.figure(figsize=(14, 8), facecolor=NAVY)
    title(fig, "Backtest Surface: SMA Fast / Slow Parameter Robustness", "Backtester output across a parameter grid, showing Sharpe stability instead of a single optimized point")
    ax = fig.add_subplot(111, projection="3d")
    style_3d(ax)
...
def plot_liquidity_slippage_surface_3d(out: Path) -> None:
    ...
    fig = plt.figure(figsize=(14, 8), facecolor=NAVY)
    title(fig, "Liquidity Surface: Slippage by Order Size and Participation", "Adjusted OHLCV volume, FINRA short pressure and capacity assumptions rendered as an execution-cost surface")
    ax = fig.add_subplot(111, projection="3d")
    style_3d(ax)
...
def plot_option_greeks_surface_3d(out: Path) -> None:
    ...
    fig = plt.figure(figsize=(14, 8), facecolor=NAVY)
    title(fig, "Options Greeks Surface: Net Gamma Exposure", "Options chain, strikes, expirations and underlying price transformed into a 3D Greeks exposure surface")
    ax = fig.add_subplot(111, projection="3d")
    style_3d(ax)
